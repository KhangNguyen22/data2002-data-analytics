...
1. The first question being whether the number of covid tests collected in survey follows a Poisson distribution. Finding if the number of covid tests follows a given distribution will enable one to better predict the behaviour of DATA2002 students.

2. The second question is whether there is a dependency relationship between living with parents and the hours a week spent exercising. It is common belief that individuals living at home have greater cost savings, both in terms of monetary and time, hence a relationship would confirm this belief for DATA2002 students.

3. 

This report uses a hypothesis testing framework to answer the above questions.
...
Question: Is there a correlation between DATA2002 students who live with their parents and the hours spent exercising per week?


**Do a chi-square hypothesis test of Independence**
To test this question, we will use a **chi-square hypothesis test of Independence**. The dataframe used in this question will be df_exercising instead of df_clean, because in df_exercising, the null values for exercising column have been removed.

WHY? - To use a test of independence, we need one population (DATA2002 students) and 2 categorical variables (lives with their parents & exercising). We want to test if the 2 variables are independent, hence the test of independence is most appropriate.
...
```
In Figure 4, we can see our observed cell table. This table is used to generate the figure 5 Expected table by using the following formula:
...
From Figure 5, we see that there are some expected cell counts that are less than 5, hence we should merge columns, opting for right to left merge.


```{r 2_p_value, message=FALSE}

chisq.test(df_exercising$live_with_parents,df_exercising$exercising)
```
...
All we know is there is a correlation between the 2 variables in the dataset, but we don't know what causes it.
Limitations of data
