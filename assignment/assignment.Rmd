...
3. The third question focuses on whether the number of covid tests is independent from a student's stress level. This question is important as it is natural to believe that a stressful person is more likely to do covid tests. To answer this question, we will use a **Chi-square test of independence**.
...
df_stress <- clean_df[is.na(clean_df$stress_level)==FALSE,]
...
* After cleaning, there are 3 main data frames used in this report.
  ...
  3. **df_stress** = This data frame is based on the clean_df and contains only **170 rows** of observations due to the removal of NA values in the stress_level variable. This dataframe will be used in **section 4.3**.

I only fully cleaned variables (stress_level and exercising) for which I will use for our data analysis by removing null values and checking if the values of the variable made sense. 
...
### Is there a relationship between number of covid tests and stress level?

**Do a chi-square hypothesis test of Independence**

Rationale: Due to influx of covid-19 cases, more people are stressed and worried about becoming infected. There is likely to be some correlation of the stress level of DATA2002 student and the number of times one goes testing. However, since testing is voluntary, we do not know how many students will go testing. Doing a hypothesis test will help us determine if stress can be a factor in influencing the number of times one gets tested for covid.


To answer this question, we will use a chi-square hypothesis test of Independence. We will use the **df_stress** dataframe, which contains 170 rows of observations due to removal of NA values in the stress_level variable.

**Hypothesis test**

$H_0:$ The number of covid tests is independent of a student's stress level.

$H_1:$ The number of covid tests is not independent of a student's stress level.
...
```{r s, message=FALSE, error=FALSE}
# table(df_stress$covid_tests)
t <- table(df_stress$covid_tests,df_stress$stress_level)
rt <-  chisq.test(t)
a <- rt$expected[,1]+rt$expected[,2]+rt$expected[,3]+rt$expected[,4]
sum(a[3:8])
```
Data
...
**Decision**:
All we know We don't know if

As our original expected cell table had multiple cells with frequency values less than 5, any
